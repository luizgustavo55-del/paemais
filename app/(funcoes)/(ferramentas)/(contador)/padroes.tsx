...
export default function Padroes() {
  ...
  const carregarPadroes = async () => {
    ...
    let dados: RegistroChute[] = dbData?.historicoChutes || [];
  ...
  const calcularInteligencia = (dados: RegistroChute[]) => {
    ...
    const pico = Object.keys(turnos).reduce((a, b) =>
      turnos[a as keyof typeof turnos] > turnos[b as keyof typeof turnos] ? a : b
    );
    setHorarioPico(pico);
    ...
    const variacao = Math.max(...registrosRecentes) - Math.min(...registrosRecentes);
    
    setConsistencia(
      variacao < 120 ? "Consistente" : "Variável"
    );
    ...
    if (ultimo > mediaSeg * 2) {
      setAlerta("O movimento recente foi mais lento que o padrão. Observe nas próximas horas.");
    ...
    const statusConsistencia = variacao < 120 ? "um padrão consistente" : "uma leve variação nos movimentos";
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.push('/(funcoes)/(contador)/contador' as any)} style={styles.back}>
        ...
        <ScrollView
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={aoAtualizar} tintColor={theme.colors.primary} />
          }
        ...
        >
          ...
          {historico.length < 3 ? (
            <Text style={{ textAlign: 'center', marginTop: 20, color: theme.colors.title, opacity: 0.7 }}>
              Use o contador pelo menos 3 vezes para gerar os primeiros padrões do seu bebê.
            </Text>
          ) : (
            <>
              ...
              <View style={styles.grid}>
                ...
                <View style={styles.cardHalf}>
                  <Text style={[styles.valor, { fontSize: 18 }]}>{consistencia}</Text>
